...
class SleepViewController: UIViewController {
    ...
    lazy var imageString: UILabel = {
        let titleLabel = UILabel()
        titleLabel.text = "icon_sleep_blue"
        return titleLabel
    }()
    ...
    lazy var timeTextField: UITextField = {
        ...
        textField.placeholder = "   Feel Sleep"
    ...
    lazy var wokeUpTextField: UITextField = {
        ...
        timePicker.frame.size = CGSize(width: 0, height: 250)
        textField.inputView = timePicker
        timePicker.addTarget(self, action: #selector(wokeUpTextFieldChanged), for: .valueChanged)
        return textField
    }()
    ...
    @objc func wokeUpTextFieldChanged(sender: UIDatePicker){
        let wokeFormatter = DateFormatter()
        wokeFormatter.dateStyle = .long
        wokeFormatter.timeStyle = .none
        wokeFormatter.dateFormat = "HH:mm:ss"
        let birth = sender.date
        let formattedTime = wokeFormatter.string(from: birth)
        wokeUpTextField.text = formattedTime
    }
    ...
    @objc func saveButtonTapped() {
        startAnimation()
        let newActivity = ActivityModel(type: .Sleep, time: timeTextField.text ?? "", note: longTextView.text ?? "")
 ...
 func updateButtonVisibility() {
     ...
     let textField2HasText = !(wokeUpTextField.text?.isEmpty ?? true)
...
//        customViewController.titleLabel.text = "Sleep"
//        customViewController.timeTextField.placeholder = "   Fell Sleep"
//        customViewController.amountTextField.isHidden = false
//        customViewController.wokeUpTextField.placeholder = "   Woke Up "
